...
`color` prop takes preset color names from the theme (which includes `primary`, `secondary`, `success` etc.), it also can take a custom color such as `#16a054`. Same applies for `size` prop

**Note!!!** In web environment sometimes you need to wrap `Typography` components in a `<Box>` or provide a parent node with `display: flex` and `flex-direction: column` for centering to work properly. In React Native it is non issue because all nodes be default have these properties
